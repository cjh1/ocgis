...
class TestSpatialCollection(AbstractTestField):
    ...
    def test_iteration(self):
        ...
        for ii,row in enumerate(sp.get_iter()):
            if ii == 1:
                self.assertEqual(row[1],[1, 1, 1, 1, 1, 2, 'tmax', 'tmax', datetime.datetime(2000, 1, 1, 12, 0), 2000, 1, 1, 50, 0.7203244934421581])
    ...
    def test_calculation_iteration(self):
        ...
        for ii,row in enumerate(sp.get_iter()):
            if ii == 0:
                ...
                as_dict = {h:row[1][ii] for ii,h in enumerate(constants.calc_headers)}
                self.assertDictEqual(as_dict,{'lid': 1, 'ugid': 1, 'vid': 1, 'cid': 1, 'did': 1, 'year': 2000, 'time': datetime.datetime(2000, 1, 16, 0, 0), 'calc_alias': 'my_mean_tmax', 'value': 0.44808476666433006, 'month': 1, 'alias': 'tmax', 'variable': 'tmax', 'gid': 1, 'calc_key': 'mean', 'tid': 1, 'level': 50, 'day': 16})
            self.assertEqual(len(row),2)
    ...
    def test_calculation_iteration_two_calculations(self):
        ...
        for ii,row in enumerate(sp.get_iter()):
            ...
            if ii == 0:
                self.assertEqual(row[0].bounds,(-100.5, 39.5, -99.5, 40.5))
                self.assertEqual(row[1],[1, 1, 1, 1, 1, 1, 1, 'tmax', 'tmax', 'mean', 'my_mean_tmax', datetime.datetime(2000, 1, 16, 0, 0), 2000, 1, 16, 50, 0.44808476666433006])
            self.assertEqual(len(row),2)
            self.assertEqual(len(row[1]),len(constants.calc_headers))
